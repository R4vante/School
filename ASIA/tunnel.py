...
class Wave_Packet:
    def __init__(self, n_points, dt, sigma0=5.0, k0=1.0, x0=-150.0, x_begin=-200.0,
        ...
        self.psi = norm*np.exp(-(self.x - x0)**2 / (4.0 * sigma0**2))*np.exp(1.0j * k0 * self.x)
...
class Animator:
    ...
    def animate(self):
        self.ani = animation.FuncAnimation(
            self.fig, self.update, frames = self.time_step, interval=5, blit=False)
        
        self.ani.save('tunnel.gif', fps=30)
 
 
wave_packet = Wave_Packet(n_points=500, dt=0.5, barrier_width=10, barrier_height=0.5)
animator = Animator(wave_packet)
animator.animate()
# animator.ani.save('tunnel.mp4')
